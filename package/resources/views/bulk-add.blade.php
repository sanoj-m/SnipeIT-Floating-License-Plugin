                    ...
                    <div class="form-group{{ $errors->has('user_ids') ? ' has-error' : '' }}">
                        ...
                        <div id="bulkUserList" style="max-height:400px; overflow-y:auto; border:1px solid #d2d6de; border-radius:4px; padding:4px 0;">
                            ...
                            <div class="bulk-user-row" style="display:flex; align-items:center; gap:8px; padding:7px 14px; border-bottom:1px solid #f2f2f2;" data-search="{{ strtolower($user->display_name . ' ' . $user->username) }}">
                                <input type="checkbox" name="user_ids[]" value="{{ $user->id }}" id="bulk-user-{{ $user->id }}" class="bulk-user-checkbox" style="position:static; margin:0;">
                                <label for="bulk-user-{{ $user->id }}" style="margin:0; font-weight:normal;">{{ $user->display_name }} ({{ $user->username }})</label>
                            </div>
                            @endforeach
                        </div>
